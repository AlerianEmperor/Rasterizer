...
struct Scene
{
	...
	Scene(string file_name, int w_, int h_)
	{
		...
		//light_direction.x *= DIRECTIONAL_LIGHT_DISTANCE;
		//light_direction.z *= DIRECTIONAL_LIGHT_DISTANCE;

		//light_position = vec3(light_direction.x * cosf(angle), light_direction.y * DIRECTIONAL_LIGHT_DISTANCE, light_direction.z * sinf(angle));
	...
	void update_light(Camera*& shadow_camera)
	{
		...
		close_light_position = l * 10.0f;//l * light_distance_scale;

		//light_position.x = light_direction.x * cosf(angle);
		//light_position.z = light_direction.z * sinf(angle);

		//l = (light_position - center).norm();
		//light_direction = (light_position - center).norm();
		...
		//close_light_position = light_position * light_scale;
	...
	float in_shadow(Camera*& camera, vec3& world_point, vec3& normal)
	{
		...
		//vec3 lp = light_position - (world_point + normal * 0.002f);

		//double current_depth = sqrtf(lp.x * lp.x + lp.y * lp.y + lp.z * lp.z);

		double bias = maxf(0.05f * (1.0f - abs(dot(normal, light_direction))), 0.005f);

		//double bias = base_bias + slope_factor * maxf(0, dot(normal, light_direction));
	...
	void __fastcall render_scene(Camera*& camera, int w_, int h_, image& img, vector<float>& z_buffer, bool render_pass)
	{
		...
		for (int i = 0; i < mesh.num_triangles; ++i)
		{
			...
			//int bmin_y = (int)bmin.y;


			//float base_u = (p02x * (bmin_y - points[0].y) + (bmin.x - points[0].x) * p02y),
			//	base_v = ((bmin.x - points[0].x) * p01y + p01x * (points[0].y - bmin_y));

			#pragma omp parallel for schedule(guided)
			for (int y = bmin.y; y <= bmax.y; ++y)
			{
				//float u = base_u + (y - bmin_y) * p02x;
				//float v = base_v - (y - bmin_y) * p01x;
				...
				for (uint32_t x = bmin.x; x <= bmax.x; ++x)
				{
					...
					if (render_pass)
					{
						//vec3 n = (1.0f - u - v) * normals[0] + u * normals[1] + v * normals[2];
