...
struct Mesh
{
	...
	//vector<glm::vec3i> vertex_indices;
	//vector<glm::vec3i> texcoord_indices;
	//vector<glm::vec3i> normal_indices;
...
Mesh load_obj(string file_name)
{
	...
	mesh.tangents.resize(num_face, glm::vec3(0));//(mesh.vertex_indices.size(), 0);
	mesh.biTangents.resize(num_face, glm::vec3(0));//(mesh.vertex_indices.size(), 0);

	for (int i = 0; i < num_face; ++i)//mesh.vertex_indices.size(); ++i)
	{
		/*glm::vec3i index = mesh.vertex_indices[i];

		glm::vec3 p0 = mesh.vertices[index.x];
		glm::vec3 p1 = mesh.vertices[index.y];
		glm::vec3 p2 = mesh.vertices[index.z];

		glm::vec3i tex_index = mesh.texcoord_indices[i];

		glm::vec2 uv0 = mesh.texcoords[tex_index.x];
		glm::vec2 uv1 = mesh.texcoords[tex_index.y];
		glm::vec2 uv2 = mesh.texcoords[tex_index.z];
		*/
		...
		glm::vec3 p0 = mesh.vertices[index0];
		glm::vec3 p1 = mesh.vertices[index1];
		glm::vec3 p2 = mesh.vertices[index2];
		...
		glm::vec2 uv0 = mesh.texcoords[tex_index0];
		glm::vec2 uv1 = mesh.texcoords[tex_index1];
		glm::vec2 uv2 = mesh.texcoords[tex_index2];


		glm::vec3 dp1 = p1 - p0;
		glm::vec3 dp2 = p2 - p0;
		glm::vec2 dt1 = uv1 - uv0;
		glm::vec2 dt2 = uv2 - uv0;
	...
	for (int i = 0; i < mesh.tangents.size(); ++i)
	{
		...
		//glm::vec3 t = mesh.tangents[i].norm();

		//mesh.tangents[i] = (t - n * dot(n, t)).norm();
		mesh.tangents[i] = normalize(mesh.tangents[i]);
		mesh.biTangents[i] = normalize(mesh.biTangents[i]);
	}
